...
import { getMouvements, verifyMouvementOnBlockchain } from './actions';
...
export default function MouvementsPage() {
  ...
  const [blockchainVerifications, setBlockchainVerifications] = useState<Record<number, boolean | null>>({});
  ...
  // Vérifier un mouvement sur la blockchain
  const handleVerifyOnBlockchain = async (mouvementId: number, event: React.MouseEvent) => {
    event.stopPropagation();
    
    setVerifyingHash(mouvementId);
    
    try {
      const result = await verifyMouvementOnBlockchain(mouvementId);
      
      setBlockchainVerifications(prev => ({
        ...prev,
        [mouvementId]: result.verified
      }));
      
      if (result.verified) {
        console.log('✅ Mouvement vérifié sur blockchain:', result);
      }
    } catch (error) {
    ...
    mouvements.forEach(mouvement => {
      ...
      if (!groups.has(lotId)) {
        ...
        groups.set(lotId, {
          ...
          blockchainLotId: mouvement.lot.blockchain_lot_id || null,
        });
      }
  ...
  const exportData = () => {
    ...
    const data = allMouvements.map(m => ({
      ...
      blockchain_tx: m.transaction_hash || '',
      blockchain_verified: m._blockchain?.verified_on_blockchain ? 'Oui' : 'Non'
    }));
    ...
    <div className="max-w-7xl mx-auto px-6 lg:px-8 py-8">
      <div className="mb-10">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Traçabilité complète avec vérification blockchain
        </p>
      </div>
      ...
      {/* Cartes statistiques */}
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-px bg-slate-200 mb-8">
        ...
        <div className="bg-white p-5">
          <div className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Sur blockchain</div>
          <div className="text-2xl font-semibold text-purple-600">
            {groupedMouvements.filter(g => g.blockchainLotId).length}
          </div>
        </div>
        <div className="bg-white p-5">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-4 mt-1 text-xs">
                            ...
                            {/* Badge blockchain du lot */}
                            {group.blockchainLotId && (
                              <>
                                <span className="text-slate-300">|</span>
                                <span className="inline-flex items-center gap-1 text-purple-600">
                                  <FaCube className="h-3 w-3" />
                                  <span className="font-mono">#{group.blockchainLotId}</span>
                                </span>
                              </>
                            )}
                          </div>
                  ...
                  <div className="flow-root">
                    <ul className="-mb-8">
                      {group.mouvements.map((mouvement, index) => {
                        ...
                        const hasBlockchain = mouvement._blockchain?.verified_on_blockchain || false;
                        const blockchainTxHash = mouvement._blockchain?.transaction_hash || mouvement.transaction_hash;
                        const isVerifying = verifyingHash === mouvement.id;
                        const isVerified = blockchainVerifications[mouvement.id];
                            ...
                            <div className="relative pb-8">
                              ...
                              <div className="relative flex gap-4">
                                ...
                                <div className="flex min-w-0 flex-1 justify-between gap-6 pt-0.5">
                                  ...
                                  {/* Partie droite : date et blockchain */}
                                  <div className="whitespace-nowrap text-right">
                                    ...
                                    <div className="mt-2 space-y-1.5">
                                      {/* Statut de vérification blockchain */}
                                      {hasBlockchain ? (
                                        <span className="inline-flex items-center bg-purple-50 px-2.5 py-1 text-xs font-medium text-purple-700 border border-purple-200">
                                          <FaShieldAlt className="mr-1.5 h-3 w-3" />
                                          Vérifié on-chain
                                        </span>
                                      ) : blockchainTxHash ? (
                                        <span className="inline-flex items-center bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700 border border-amber-200">
                                          <FaCube className="mr-1.5 h-3 w-3" />
                                          Tx: {blockchainTxHash.substring(0, 8)}...
                                        </span>
                                      ) : (
                                        <span className="inline-flex items-center bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-500 border border-slate-200">
                                      ...
                                      {mouvement.hash_mouvement && (
                                        <div className="font-mono text-xs text-slate-400" title={mouvement.hash_mouvement}>
                                          Hash: {mouvement.hash_mouvement.substring(0, 10)}...
                                        </div>
                                      )}
                                      
                                      {/* Transaction blockchain */}
                                      {/* {blockchainTxHash && (
                                        <div className="font-mono text-xs text-purple-500" title={blockchainTxHash}>
                                          <a 
                                            href={`https://etherscan.io/tx/${blockchainTxHash}`}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="hover:text-purple-700 inline-flex items-center gap-1"
                                      ...
                                      {/* Transaction blockchain - Ganache local */}
{/* ✅ CORRECTION : Afficher le numéro de bloc */}
{blockchainTxHash && (
  <div className="space-y-1">
    <div 
      className="font-mono text-xs text-purple-500 cursor-pointer hover:text-purple-700 inline-flex items-center gap-1"
      title={`Transaction Ganache: ${blockchainTxHash}\nCliquez pour copier`}
    >
      <FaCube className="h-2.5 w-2.5" />
      <span>Tx: {blockchainTxHash.substring(0, 10)}...</span>
      <span className="text-purple-300 text-[10px]">(copier)</span>
    </div>
    <div className="text-[10px] text-slate-400">
      Ganache local • Bloc #{mouvement._blockchain?.block_number || '—'}
    </div>
  </div>
)}
                                      
                                      {/* ID Blockchain du lot */}
                                      {mouvement._blockchain?.blockchain_lot_id && (
                                        <div className="text-xs text-purple-400 font-mono" title={`Blockchain Lot ID: ${mouvement._blockchain.blockchain_lot_id}`}>
                                          <FaCube className="inline h-2.5 w-2.5 mr-1" />
                                          Lot #{mouvement._blockchain.blockchain_lot_id}
                                        </div>
                                      )}
                                      
                                      {/* Bouton de vérification blockchain */}
                                      <button
                                        onClick={(e) => handleVerifyOnBlockchain(mouvement.id, e)}
                                        disabled={isVerifying || isVerified === true}
                                        className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-medium border transition-colors mt-1
                                          ${isVerified === true
