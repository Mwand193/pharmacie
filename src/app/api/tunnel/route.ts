...
// //       const child = exec(
// //         `cloudflared tunnel --url http://localhost: --no-autoupdate`,
// //         {
// //           windowsHide: true,
    ...
    return new Promise<NextResponse>((resolve) => {
      const child = exec(
        `cloudflared tunnel --url http://localhost:3000 --no-autoupdate`,
        {
          windowsHide: true,
          maxBuffer: 1024 * 1024 * 10,
          // Ajouter le dossier de cloudflared au PATH
          env: {
            ...process.env,
            PATH: `C:\\Program Files (x86)\\cloudflared;${process.env.PATH}`
          }
        }
      );
